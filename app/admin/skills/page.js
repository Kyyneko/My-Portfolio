...
import { getSkills, upsertSkill, deleteSkill } from '@/lib/data';
...
export default function AdminSkills() {
    ...
    // Group for display
    const grouped = items.reduce((acc, s) => { const c = s.category || 'Other'; if (!acc[c]) acc[c] = []; acc[c].push(s); return acc; }, {});
    ...
    return (
        <div>
            ...
            {items.length === 0 && <p style={{ color: 'var(--text-muted)', textAlign: 'center', padding: '2rem' }}>No skills yet.</p>}
